...
async function loadAlbums() {
    ...
    // 显示加载状态
    if (loadingManager) {
        loadingManager.showLoading('正在加载相册...');
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log('加载到的相册数据:', allAlbums);
            console.log('相册数量:', allAlbums.length);
            ...
            // 显示成功通知
            if (feedbackSystem && allAlbums.length > 0) {
                feedbackSystem.showNotification({
                    type: 'success',
                    title: '加载完成',
                    message: `成功加载 ${allAlbums.length} 个相册`,
                    duration: 2000
                });
            }
        } else {
...
// 分享相册
async function shareAlbum(id, title) {
